...
f=@(x,y) f2(x,y,t);
L=L_b;
[u,Au]=FEMParabolic2DP1Richards(Coordinates,Elements,L,0,Dirichlet,DirichletValue,Neumann,g,f,b(u_0)-b(u_0),1,0,Coefficients,GaussValues,bprime,u_0,SCHEME,bprimemax/2,KIRCHHOFF,kappa,kappaprime,tau);
u_old = u_0;
...
while error > 10^(-8)
        u_prev=u;
        [u,Au]=FEMParabolic2DP1Richards(Coordinates,Elements,L,0,Dirichlet,DirichletValue,Neumann,g,f,b(u_old)-b(u_prev),0,Au,Coefficients,GaussValues,bprime,u_prev,SCHEME,bprimemax/2,KIRCHHOFF,kappa,kappaprime,tau);
...
legend([p1,p2,p3,p4],'L=L_b','Modified L - m=max(bprime)/2','Newton','Locally optimized L')
title('\kappa = 1, \tau=0.1, L_b=10, b_m=1, h=1/8')
