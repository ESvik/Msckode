%Richards Eq in 2D Order of convergence
...
% p1=plot(1:length(nonzeros(Errors(1,:))),nonzeros(Errors(1,:)))
% hold on
% p2=plot(1:length(nonzeros(Errors(2,:))),nonzeros(Errors(2,:)))
% p3=plot(1:length(nonzeros(Errors(3,:))),nonzeros(Errors(3,:)))
% p4=plot(1:length(nonzeros(Errors(4,:))),nonzeros(Errors(4,:)))
...
legend([p1,p2,p3,p4],'L=L_b','Modified L - m=max(bprime)/2','Newton','Locally optimized L')
ylabel('log(||u_i-u||/||u_{i-1}-u||)/log(||u_{i-1}-u||/||u_{i-2}-u||)')
xlabel('Iteration i')
title('\kappa = 10, \tau=0.1, L_b=10, b_m=1, h=1/16')
