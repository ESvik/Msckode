...
function [u,A] = FEMLinStrain2DvectorialP2Mandel(Coordinates,Elements,C,B,Dirichlet,Neumann,g,f,h,fVector,firstrun,Apre,Coefficients,GaussValues,Nx,Ny)
    NN=length(Coordinates(:,1));
    b=zeros(2*(2*Nx-1)*(2*Ny-1),1);
...
%     end
    ...
    if firstrun == 1
        A=sparse(2*(2*Nx-1)*(2*Ny-1),2*(2*Nx-1)*(2*Ny-1));
