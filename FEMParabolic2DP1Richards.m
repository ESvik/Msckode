...
function [u,A] = FEMParabolic2DP1Richards(Coordinates,Elements,C,B,Dirichlet,DirichletValue,Neumann,g,f,fVector,firstrun,Apre,Coefficients,GaussValues,bprime,u_prev,scheme,modifiedconstant,Kirchhoff,kappa,kappaprime,tau)
...
%% -------------------------------------------------------------------------------
    ...
    if firstrun == 1
        for k = 1:number_of_elements
            ...
            for i =1:3
                for j = 1:3
                    ALdiv(i,j) = 1/6*[Coefficients(i,2),Coefficients(i,3)]*(ARefTriInv*ARefTriInv')*[Coefficients(j,2);Coefficients(j,3)]*det(ARefTri)*sum(Kappa);
                end
            end
    ...
    if scheme == 3
        for k = 1:number_of_elements
            ...
            ARefTriInv = ARefTri^(-1);
            GaussNodes= [ARefTri*[1/6;1/6]+bRefTri,ARefTri*[2/3;1/6]+bRefTri,ARefTri*[1/6;2/3]+bRefTri];
            MVectCoeff=[1,vertices(1,1),vertices(1,2);1,vertices(2,1),vertices(2,2 );1,vertices(3,1),vertices(3,2)]\(bprime(u_prev(nodes))+kappaprime(u_prev(nodes)));
            M = [MVectCoeff'*[1;GaussNodes(:,1)],MVectCoeff'*[1;GaussNodes(:,2)],MVectCoeff'*[1;GaussNodes(:,3)]];
    ...
    if scheme == 4
        ...
        for k = 1:number_of_elements
            ...
            gamma = max(0,1+(Lb*bm-sqrt(Lb^2*bm^2+4*Lb*bm^2*tau*kappamin+4*Lb*bm*(tau*kappamin)^2))/(4*bm*tau*kappamin));
            L=Lb/(2*(1-gamma));
